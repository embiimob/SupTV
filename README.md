...
It auto-loads trending search stats from `https://p2fk.io/GetTrendingRootSearches?qty=100` (matching `TRENDING_STATS_LIMIT` in `index.html`) and then selects the top 20 unique search strings for playback (`TRENDING_KEYWORD_LIMIT`).
...
The bottom ticker banner is expandable: click it to open a queue/trending panel, showing queued videos in top-down playback order with a 20-at-a-time **Show more** flow plus up to top 100 trending searches (click any search to run it), then click outside to collapse.
Trending ticker crawl text includes search-term stats with result totals when trending data is available (showing `200+` when a search reaches the default max fetch cap).
When SupTV is in the default trending mode, it now keeps the station on-air by following exhausted trending results with the top 20 `mp4` results, then a fresh trending cycle, repeating in that order.
